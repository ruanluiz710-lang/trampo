...
export default function Profile() {
  ...
  return (
    <div style={{ ...style, width: '100%', minHeight: '100vh', backgroundColor: '#F5F5F5' }}>
      ...
      {/* Header escuro */}
      <div style={{ backgroundColor: '#1C1C1C', padding: '48px 32px 80px', width: '100%' }}>
        <div style={{ maxWidth: '860px', margin: '0 auto' }}>
          <Link to="/profissionais" style={{ color: 'rgba(255,255,255,0.5)', fontSize: '14px', fontWeight: '500', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '6px', marginBottom: '32px' }}>
            ← Voltar para a busca
          </Link>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '24px', flexWrap: 'wrap' }}>
            {/* Avatar */}
            <div style={{ width: '100px', height: '100px', borderRadius: '24px', backgroundColor: '#FAECE7', border: '4px solid rgba(255,255,255,0.1)', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              {professional.photo_url
                ? <img src={professional.photo_url} alt={professional.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                : <span style={{ color: '#D85A30', fontWeight: '900', fontSize: '36px' }}>{professional.name?.[0]}</span>
              }
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap', marginBottom: '6px' }}>
                <h1 style={{ color: '#fff', fontWeight: '900', fontSize: '2rem', margin: 0 }}>{professional.name}</h1>
                {professional.professional_services?.[0]?.categories?.name && (
                  <span style={{ backgroundColor: '#D85A30', color: '#fff', fontSize: '12px', fontWeight: '700', padding: '4px 12px', borderRadius: '999px' }}>
                    {professional.professional_services[0].categories.name}
                  </span>
                )}
              </div>
              <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '15px', margin: 0 }}>📍 {professional.city}, {professional.state}</p>
            </div>
            {avgRating && (
      ...
      <div style={{ maxWidth: '860px', margin: '-32px auto 0', padding: '0 32px 64px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '24px', alignItems: 'start' }}>
              ...
              <div style={{ marginTop: '24px', paddingTop: '24px', borderTop: '1px solid #f5f5f5' }}>
                ...
                {reviewSuccess ? (
                  ...
                  <form onSubmit={submitReview} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <input
                      type="text" placeholder="Seu nome" value={reviewForm.client_name}
                    ...
                    <textarea
                      placeholder="Conte como foi a experiência (opcional)" value={reviewForm.comment}
          ...
          <div style={{ position: 'sticky', top: '88px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{ backgroundColor: '#fff', borderRadius: '20px', padding: '28px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
              <p style={{ fontWeight: '700', fontSize: '13px', color: '#9ca3af', marginBottom: '16px', letterSpacing: '0.05em' }}>CONTATO</p>
              <a
                href={whatsappUrl} target="_blank" rel="noopener noreferrer"
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', backgroundColor: '#25D366', color: '#fff', fontWeight: '800', fontSize: '16px', padding: '16px', borderRadius: '14px', textDecoration: 'none', marginBottom: '12px' }}
              >
              ...
              <p style={{ textAlign: 'center', fontSize: '12px', color: '#9ca3af' }}>
                Clique e fale diretamente com {professional.name.split(' ')[0]}
              </p>
            </div>
